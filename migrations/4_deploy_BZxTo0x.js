...
module.exports = function(deployer, network, accounts) {
	...
	deployer.deploy(BZxTo0x, config["addresses"][network]["ZeroEx"]["ExchangeV1"], config["addresses"][network]["ZeroEx"]["ZRXToken"], config["addresses"][network]["ZeroEx"]["TokenTransferProxy"]).then(async function(bZxTo0x) {
		/*if (network != "mainnet" && network != "ropsten" && network != "kovan" && network != "rinkeby") {
			await bZxTo0x.setDebugMode(true);
		}*/

		await deployer.deploy(BZxTo0xV2, config["addresses"][network]["ZeroEx"]["ExchangeV2"], config["addresses"][network]["ZeroEx"]["ZRXToken"], config["addresses"][network]["ZeroEx"]["ERC20Proxy"]);
