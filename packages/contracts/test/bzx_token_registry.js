...
contract('TokenRegistry', function (accounts) {
    ...
    context("Register", async () => {
        ...
        it('shouldn\'t allow stranger to add token', async () => {
            try {
                await tokenRegistry.addToken(token1.address, NAME, SYMBOL, DECIMALS, "uri", {from: stranger});
                assert.isTrue(false);
            } catch (e) {
        ...
        it('should allow owner to add token', async () => {
            await tokenRegistry.addToken(token1.address, NAME, SYMBOL, DECIMALS, "uri");
        ...
        it('shouldn\'t allow owner to register the same token (address/name/symbol) twice', async () => {
            ...
            try {
                await tokenRegistry.addToken(token1.address, name2, symbol2, decimals2, "uri2");
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
            ...
            try {
                await tokenRegistry.addToken(token2.address, NAME, symbol2, decimals2, "uri2");
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
            ...
            try {
                await tokenRegistry.addToken(token2.address, name2, SYMBOL, decimals2, "uri2");
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }

            await tokenRegistry.addToken(token2.address, name2, symbol2, decimals2, "uri2");
            await ensureTokenExists(token2.address, name2, symbol2, decimals2, "uri2");
        });
        ...
        it('shouldn\'t allow owner to register 0x0 address', async () => {
            ...
            await tokenRegistry.addToken(token3.address, name3, symbol3, decimals3, "uri3");
            await ensureTokenExists(token3.address, name3, symbol3, decimals3, "uri3");
        });
    ...
    context("Remove", async () => {
        ...
        before('before', async () => {
            ...
            await tokenRegistry.addToken(token1.address, NAME1, SYMBOL1, DECIMALS1, URI1);
            await tokenRegistry.addToken(token2.address, NAME2, SYMBOL2, DECIMALS2, URI2);
        })
        ...
        it('shouldn\'t allow stranger to remove token', async () => {
            ...
            try {
                await tokenRegistry.removeToken(token1.address, index, {from: stranger});
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
    ...
    context("Alter", async () => {
        ...
        before('before', async () => {
            ...
            await tokenRegistry.addToken(token1.address, NAME1, SYMBOL1, DECIMALS1, URI1);
            await tokenRegistry.addToken(token2.address, NAME2, SYMBOL2, DECIMALS2, URI1);
        })

        it('should allow owner to update token\'s symbol', async () => {
            try {
                await tokenRegistry.setTokenSymbol(token1.address, SYMBOL_UPD, {from: stranger});
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
            ...
            await ensureTokenExists(token1.address, NAME1, SYMBOL_UPD, DECIMALS1, URI1);
        });

        it('should allow owner to update token\'s name', async () => {
            try {
                await tokenRegistry.setTokenName(token1.address, NAME_UPD, {from: stranger});
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
            ...
            await ensureTokenExists(token1.address, NAME_UPD, SYMBOL_UPD, DECIMALS1, URI1);
        });

        it('should allow owner to update token\'s uri', async () => {
            try {
                await tokenRegistry.setTokenURL(token1.address, URI_UPD, {from: stranger});
                assert.isTrue(false);
            } catch (e) {
                utils.ensureException(e);
            }
            ...
            await ensureTokenExists(token1.address, NAME_UPD, SYMBOL_UPD, DECIMALS1, URI_UPD);
        });
    ...
    context("Token list consistency", async () => {
        it('should always has consistent token list', async () => {
            ...
            await tokenRegistry.addToken(token1.address, "NAME1", "SYMBOL1", 8, "URI1");
            assert.equal((await tokenRegistry.getTokenAddresses.call()).length, 1);

            await tokenRegistry.addToken(token2.address, "NAME2", "SYMBOL2", 8, "URI2");
    ...
    let ensureTokenExists = async (address, name, symbol, decimals, uri) => {
        assert.equal(address, await tokenRegistry.getTokenAddressBySymbol.call(symbol));
    ...
    let ensureTokenDoesNotExist = async (address, name, symbol) => {
        assert.equal(utils.zeroAddress, await tokenRegistry.getTokenAddressBySymbol.call(symbol));
        assert.equal(utils.zeroAddress, await tokenRegistry.getTokenAddressByName.call(name));
