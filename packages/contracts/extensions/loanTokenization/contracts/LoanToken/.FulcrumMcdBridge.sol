...
library PartialFulcrum {
    ...
    function mint(IIToken _iToken, address _receiver, uint256 _amount) internal returns (uint256 mintAmount) {
        ...
        mintAmount = _iToken.mint(_receiver, amount);
    }

    function burn(IIToken _iToken, address _receiver, uint256 _amount) internal returns (uint256 amountPaid) {
        uint256 amount = Utils.balanceOrAmount(IERC20(address(_iToken)), _amount);
        amountPaid = _iToken.burn(_receiver, amount);
        require(amountPaid >= amount.mul(_iToken.tokenPrice()).div(10**18) - 1, "FULCRUM_NOT_LIQUID");
    }
...
contract FulcrumMcdBridge is TokenSaver {
    ...
    function bridgeISaiToIDai(uint256 _amount) external returns (uint256 mintAmount) {
        ...
        mintAmount = PartialFulcrum.mint(IIToken(iDAI), msg.sender, MAX);
    }

    function bridgeIDaiToISai(uint256 _amount) external returns (uint256 mintAmount) {
        ...
        mintAmount = PartialFulcrum.mint(IIToken(iSAI), msg.sender, MAX);
    }
}
