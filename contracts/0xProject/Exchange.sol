...
contract Exchange {
    ...
    */
          ...
          returns (uint filledTakerTokenAmount)
    {
        ...
        uint remainingTakerTokenAmount = SafeMath.sub(order.takerTokenAmount, getUnavailableTakerTokenAmount(order.orderHash));
        ...
        returns (uint)
    {
        ...
        uint remainingTakerTokenAmount = SafeMath.sub(order.takerTokenAmount, getUnavailableTakerTokenAmount(order.orderHash));
        uint cancelledTakerTokenAmount = Math.min256(cancelTakerTokenAmount, remainingTakerTokenAmount);
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (order.feeRecipient != address(0)) {
            ...
            uint requiredMakerZRX = isMakerTokenZRX ? SafeMath.add(fillMakerTokenAmount, paidMakerFee) : paidMakerFee;
            uint requiredTakerZRX = isTakerTokenZRX ? SafeMath.add(fillTakerTokenAmount, paidTakerFee) : paidTakerFee;
    ...
    function getAllowance(address token, address owner)
        ...
        returns (uint)
    {
        return EIP20(token).allowance.gas(EXTERNAL_QUERY_GAS_LIMIT)(owner, TOKEN_TRANSFER_PROXY_CONTRACT); // Limit gas to prevent reentrancy
    }
}
