...
contract('OracleRegistry', function (accounts) {
    ...
    let ensureOracleExists = async (oracle, name) => {
        ...
        assert.equal(oracle, await oracleRegistry.getOracleAddressByName.call(name));
    ...
    let ensureOracleDoesNotExist = async (oracle, name) => {
        ...
        assert.equal(utils.zeroAddress, await oracleRegistry.getOracleAddressByName.call(name));
